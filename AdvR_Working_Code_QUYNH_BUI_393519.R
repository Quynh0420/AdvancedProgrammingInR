...
library(profvis)
library(ggplot2)
source("function_install.RtoolsPkgbuild.R")
install.RtoolsPkgbuild()


SP500 <- getSymbols("^GSPC",
                    # if we don't want the automatic
                    # name assignment
                    auto.assign = FALSE,
...
cppFunction('std::vector<double> myEMA(std::vector<double> x, int window_size) {

            int len = x.size();
            std::vector<double> result(len);
            if (len == 0 || len < window_size) return result;
            std::vector<double>::iterator iter = result.begin();

            double multiplier =  2.0 / (window_size + 1.0);
            ...
            *(iter + window_last_index) = sum/window_size;

            } else{

            *(iter + window_last_index) = (x[window_last_index]- *(iter + window_last_index - 1))* multiplier + *(iter + window_last_index - 1);
...
(compare_1 <- benchmark("Function_1" = {f1 <- Double_breakout_1(SP500["2010/2018", 1], strategy = "mr")},
                        "Function_2" = {f2 <- Double_breakout_2(SP500["2010/2018", 1], strategy = "mr")},
...
(compare_2 <- microbenchmark("Function_1" = {f1 <- Double_breakout_1(SP500["2010/2018", 1], strategy = "mr")},
                            "Function_2" = {f2 <- Double_breakout_2(SP500["2010/2018", 1], strategy = "mr")},
                            unit = "relative" # other possible: ns, us, ms, s
                            )
)

autoplot(compare_2, log = FALSE)
boxplot(compare_2)
...
profile_1 <- profvis(
  
  {
    ...
    MA_size = 20 # window size for EMA,
    Vol_size = 60 # window size for rolling standard deviation,
    m = 1.5 # multiplier for entry threshold,
    n = 1 # multiplier for exit threshold, should be lower than m
    trans_cost = 0.0001 # as % of price
    strategy = "mr"
...
save(list = c("f1", "f2", "SP500",
              "compare_1", "compare_2", 
              "profile_1", "profile_2"),
     file = "all_myresult.RData")
